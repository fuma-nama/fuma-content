...
const inputGroupAddonVariants = cva(
  "text-muted-foreground h-auto gap-2 py-2 text-sm font-medium group-data-[disabled=true]/input-group:opacity-50 [&>svg:not([class*='size-'])]:size-4 flex cursor-text items-center justify-center select-none",
  {
    variants: {
      align: {
        "inline-start": "pl-3 has-[>button]:ml-[-0.25rem] order-first",
        "inline-end": "pr-3 has-[>button]:mr-[-0.25rem] order-last",
        "block-start":
...
function InputGroupText({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "flex items-center gap-2 text-muted-foreground text-sm [&_svg]:size-4",
        className,
      )}
      {...props}
    />
